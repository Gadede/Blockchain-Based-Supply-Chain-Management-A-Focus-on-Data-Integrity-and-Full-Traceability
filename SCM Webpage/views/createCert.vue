              ...
              >
                <el-col :span="10">
                  ...
                  <el-form-item label="Product quantity" prop="quantity">
                    <el-input-number v-model="createCert.quantity" :min="1"></el-input-number>
                  </el-form-item>
              ...
              <el-col :span="22">
                    <fieldset>
                        <legend>Processed data</legend>
                            ...
                            <el-row>
                                <el-col :span="5" :offset="0">
                                    <p class="computedLabels">Eth Hash:</p>
                                </el-col>
                                <el-col :span="5" :offset="0">
                                    <p class="formattedString_larger">{{ethHashedData}}</p>
                                </el-col>
                            </el-row>
                            <el-row>
                              <el-col :span="5">
...
import { ABI, contractAddress, suppliedGas } from '@/assets/js/ABI'
...
export default {
  ...
  methods: {
        ...
        this.$refs[formName].validate(valid => {
          ...
          if (valid) {
            this.prodData = {
              ...
              quantity: this.createCert.quantity,
            ...
            this.prepData = this.prodData.prodID + this.prodData.prodDesc + this.prodData.quantity + this.prodData.IPFS_Hash + this.prodData.Origin +
            this.prodData.farmerSig + this.prodData.public_blockchainAddress + this.prodData.certStatus + this.prodData.certCreationTime
            getHash(this.prepData).then(res => {
    ...
    getSignatureFromValidator () {
      ...
      var ethHashedData = new web3.eth.Contract(ABI, contractAddress, { defaultGas: suppliedGas })
      // Access contract method.
      // This is call operation. Any account can be used. It costs zero Eth.
      ethHashedData.methods.getEthSignedMessageHash(this.prodDetailsHash).call({ from: this.addrOfValidator }).then(res => {
        console.log('Response from Contract: ', res)
        this.ethHashedData = res
      }).catch(err => {
        console.log('Error getting returned values: ', err)
        this.$message({
          message: 'No response received form Smart contract.',
          type: 'error'
        })
      })
    ...
    sendCertToIPFShub () {
      ...
      this.prodData.prodDetailsHash = this.ethHashedData
